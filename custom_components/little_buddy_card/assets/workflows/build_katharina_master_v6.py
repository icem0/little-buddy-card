#!/usr/bin/env python3
"""
katharina_master_v6 — v5 + PROMPT-HARDENING gegen grüne Schatten

Härtet den Prompt SO dass auch bei 10 Bildern Konsistenz:
  - Negativ: ALLE Grüntöne, Farb-Stich-Begriffe, Outdoor-Bodentypen
  - Negativ: alle Schatten-Farben außer neutral-grau
  - Positiv: explizit "neutral gray shadow, no color, desaturated, monochrome"
"""
...
PROMPT = (
    ...
    "knee-length red dress, knee length red dress, dress to knees, dress just below knees, "
    "covered legs red dress, red dress below knees, "
    "long_dress, long bright crimson red dress, full length dress, ankle-length dress, "
    ...
    "neutral gray soft round shadow under feet, desaturated shadow, monochrome shadow, "
    "no color shadow, no green shadow, no blue shadow, no colored shadow, "
    "pure white background, no scenery, isolated character, "
    "masterpiece, highly detailed, sharp focus, 8bit pixel art style"
)

NEG = (
    # === GRÜNE SCHATTEN / GRÜNTÖNE (Hauptproblem v5) ===
    "green shadow, green tint, green color, green hue, green tone, greenish, "
    "green ground, green tint, dark green, forest green, "
    "colored shadow, color shadow, tinted shadow, "
    "blue shadow, blue tint, blue color, "
    "red shadow, red tint, yellow shadow, orange shadow, purple shadow, "
    # === ALLE GRÜNTÖNE im Schatten-Bereich ===
    "grass color, grass tint, grass stain, grass reflection, "
    ...
    "shorts, mini skirt, miniskirt, crop top, "
    "ankle-length dress, floor length dress, full length dress, "
...
if __name__ == "__main__":
    ...
    print(f"=== katharina_master_v6 (PROMPT-HARDENING: kein grüner Schatten) ===")
    print(f"Base: {BASE_MODEL}, 800x800, cfg 7.0, steps 30, seed 42")
    print(f"Positiv: 'neutral gray soft round shadow, desaturated, monochrome, no green, no blue, no colored'")
    print(f"Negativ: green shadow/tint/color/hue, alle Farb-Stiche, Grass-Tints, etc.")
    print()
    ...
    out = f"{OUT_DIR}/master_v8_knee_length.png"
    print(f"\n[v6_gray_shadow] cfg=7.0, steps=30, seed=42")
    try:
    ...
    print("  Schatten jetzt neutral-grau? Rotes Kleid? Katharina?")
