...
Output: custom_components/little_buddy_card/assets/pets/level_1..5/<mood>.png  (35)
        custom_components/little_buddy_card/assets/trees/<stage>.png           (5)
"""
import os
...
def main():
    ...
    for lvl in LEVELS:
        for mood in MOODS:
            ...
            label = f"L{lvl}\n{mood}"
            p = os.path.join(ASSETS, "pets", f"level_{lvl}", f"{mood}.png")
            make_tile(p, color, label)
            count += 1
    for stage in TREES:
        color = TREE_COLORS[stage]
        p = os.path.join(ASSETS, "trees", f"{stage}.png")
        make_tile(p, color, stage)
        count += 1
    print(f"✅ {count} dummy PNGs written under {ASSETS}")
